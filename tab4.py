...
def run_tab(): 
    ...
    with tabs[0]: # 월별
        ...
        tabs[0].write(f"📢 민원 건수는 <strong>총 { t4_df0_0_temp[ 'NUMBER' ].sum() } 건</strong> 이며, 최다 발생 기간은 <strong>{ t4_df0_0_temp.iloc[0][ 'DATE' ].strftime('%Y') }년  { t4_df0_0_temp.iloc[0][ 'DATE' ].strftime('%m') }월</strong> <strong>( { t4_df0_0_temp.iloc[0][ 'NUMBER' ] } 건 )</strong> 입니다.       , ", unsafe_allow_html=True) 
