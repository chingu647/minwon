...
def run_tab(): 
    ...
    # global t4t1_map  # ----------------------------- #    map이 다르므로 함수명도 다르게 설정함
    @st.cache_resource
    def create_map_t4(point_df): 
        ...
        folium_map_t4 = t4t1_map._repr_html_() 
        st.components.v1.html(folium_map_t4, height=900) #, width=800, height=600)
    ...
    t1t0, t4t1, t1t2 = st.columns( [0.001, 0.998, 0.001] ) 

    ###################################################################### head 1  
    t4h1.markdown(f"##### {organ_t4} : 공지사항")
    t4h1.markdown(r"""
	1. 함평지사 민원은 증가추세에 있습니다.
    """)

    ###################################################################### body 1  
    t4b1.markdown("##### 2024년 이슈")

    t4b1_kind1_df, _ = load_df(organ_t4, kind1_t4) 
    ...
    t4b2_wc = load_wc(text_raw)
  

    t4b2_fig, t4b2_ax = plt.subplots(figsize=(10,4)) 
    t4b2_ax.axis('off')
    t4b2_ax.imshow(t4b2_wc)
    ...
    # -------------------------------------------------------- pie 그래프 
    # data  
    t4b5_kind1_df, _ = load_df(organ_t4, kind1_t4) 

    t4b5_x = t4b5_kind1_df.index.values
    t4b5_y = t4b5_kind1_df['건수'] 

    # preprocessing
    t4b5_fig, t4b5_ax = plt.subplots(figsize=(10,4)) 
    t4b5_ax.tick_params(
        # axis=x or axis=y,
        # labelsize=20,
        direction='inout',
    ...
    t4b5_ax.set_facecolor('white')                  # figure 배경색 

    # paint 
    explode = [0.05 for i in t4b5_x]
    wedgeprops={'width': 0.5, 'edgecolor': 'w', 'linewidth': 3}
    t4b5_ax.pie(t4b5_y, labels=t4b5_x, 
            startangle=260,
            counterclock=False, 
            autopct="%.1f%%", 
            # explode=explode,
    ...
    t4b5_cx = 0
    t4b5_cy = 0
    t4b5_val= '총' + str(t4b5_kind1_df['건수'].sum()) + '건'
    t4b5_ax.text(t4b5_cx, t4b5_cy, t4b5_val, 
                ha='center', va='center', 
                color='red',
                fontsize=23)                           # bar text 폰크 

    t4b5.pyplot(t4b5_fig) 
    ...
    # -------------------------------------------------------- 세로 bar 그래프 
    # data  
    t4b6_kind1_df, _ = load_df(organ_t4, kind1_t4) 

    t4b6_x = t4b6_kind1_df.index.values
    t4b6_y = t4b6_kind1_df['건수'] 

    # preprocessing 
    t4b6_fig, t4b6_ax = plt.subplots(figsize=(10,4)) 
    t4b6_ax.tick_params(
        # axis=x or axis=y,
        labelsize=20,
        direction='inout',
    ...
    t4b6_ax.set_facecolor('white')                  # figure 배경색 

    # paint 
    t4b6_ax.bar(t4b6_x, t4b6_y, color='#E0ECF8')            # bar plot 표시
    for i in range(len(t4b6_x)):                        # bar text 표시
        height = t4b6_y[i]+0.5 
        height_str = str(t4b6_y[i])+'건'
        t4b6_ax.text(t4b6_x[i], height, height_str, 
                 ha='center', va='bottom', 
                 color='green',
                 fontsize=20)                           # bar text 폰크 

    t4b6.pyplot(t4b6_fig) 

    ###################################################################### body 9
    t4b9.markdown("##### 노선별 민원") 
    
    # -------------------------------------------------------- 가로 sns bar 그래프 
    # data  
    t4b9_kind1_df, _ = load_df(organ_t4, kind1_t4) 
    t4b9_x = t4b9_kind1_df.index.values
    t4b9_y = t4b9_kind1_df['건수'] 

    # preprocessing ---------------------------
    t4b9_fig,  t4b9_ax = plt.subplots(figsize=(10,6)) 
    t4b9_ax.tick_params(
        # axis=x or axis=y,
        labelsize=20,
        direction='inout',
    ...
    t4b9_ax.set_facecolor('white')                  # figure 배경색 

    # paint 
    sns.barplot(x=t4b9_y, y=t4b9_x, 
                hue=t4b9_x, 
                dodge=False,
                ax=t4b9_ax) 
    for i in range(len(t4b9_x)):               # bar text 표시
        width = t4b9_y[i]+1.5 
        width_str = str(t4b9_y[i])+'건'
        t4b9_ax.text(width, i, width_str, 
                #  ha='center', va='bottom', 
                 color='green',
                 fontsize=20)                   # bar text 폰크

    t4b9.pyplot(t4b9_fig)  

    ###################################################################### body 10
    t4b10.markdown("##### 노선별 민원") 

    t4b10_kind1_df, _ = load_df(organ_t4, kind1_t4) 

    t4b10.table(t4b10_kind1_df.style.background_gradient(cmap='winter')) 
    ...
    t4t1_kind1_df, t4t1_point_df = load_df(organ_t4, kind1_t4)   
    t4t1_point_df_temp = t4t1_point_df.copy()  
    t4t1.dataframe(t4t1_point_df_temp)

    base_position = [35.18668601, 126.87954220] 

    t4t1_map = load_map(base_position) 
    # create_map(t4t1_map, t4t1_point_df) 


    create_map_t4(t4t1_point_df) 
