...
def run_tab(): 
    ...
    def create_map_t5(point_df): 
        ...
        folium_map_t5 = t5t1_map._repr_html_() 
        st.components.v1.html(folium_map_t5, height=900) #, width=800, height=600)
    ...
    t5b2_wc = load_wc(text_raw)
  

    t5b2_fig, t5b2_ax = plt.subplots(figsize=(10,4)) 
    t5b2_ax.axis('off')
    t5b2_ax.imshow(t5b2_wc)
    ...
    # preprocessing
    t5b5_fig, t5b5_ax = plt.subplots(figsize=(10,4)) 
    t5b5_ax.tick_params(
        # axis=x or axis=y,
        # labelsize=20,
        direction='inout',
    ...
    t5b5_ax.set_facecolor('white')                  # figure 배경색 

    # paint 
    explode = [0.05 for i in t5b5_x]
    wedgeprops={'width': 0.5, 'edgecolor': 'w', 'linewidth': 3}
    t5b5_ax.pie(t5b5_y, labels=t5b5_x, 
            startangle=260,
            counterclock=False, 
            autopct="%.1f%%", 
            # explode=explode,
    ...
    t5b5_cx = 0
    t5b5_cy = 0
    t5b5_val= '총' + str(t5b5_kind1_df['건수'].sum()) + '건'
    t5b5_ax.text(t5b5_cx, t5b5_cy, t5b5_val, 
                ha='center', va='center', 
                color='red',
                fontsize=23)                           # bar text 폰크 

    t5b5.pyplot(t5b5_fig) 
    ...
    # preprocessing 
    t5b6_fig, t5b6_ax = plt.subplots(figsize=(10,4)) 
    t5b6_ax.tick_params(
        # axis=x or axis=y,
        labelsize=20,
        direction='inout',
    ...
    t5b6_ax.set_facecolor('white')                  # figure 배경색 

    # paint 
    t5b6_ax.bar(t5b6_x, t5b6_y, color='#E0ECF8')            # bar plot 표시
    for i in range(len(t5b6_x)):                        # bar text 표시
        height = t5b6_y[i]+0.5 
        height_str = str(t5b6_y[i])+'건'
        t5b6_ax.text(t5b6_x[i], height, height_str, 
                 ha='center', va='bottom', 
                 color='green',
                 fontsize=20)                           # bar text 폰크 

    t5b6.pyplot(t5b6_fig) 

    ###################################################################### body 9
    t5b9.markdown("##### 노선별 민원") 
    ...
    # preprocessing ---------------------------
    t5b9_fig,  t5b9_ax = plt.subplots(figsize=(10,6)) 
    t5b9_ax.tick_params(
        # axis=x or axis=y,
        labelsize=20,
        direction='inout',
    ...
    t5b9_ax.set_facecolor('white')                  # figure 배경색 

    # paint 
    sns.barplot(x=t5b9_y, y=t5b9_x, 
                hue=t5b9_x, 
                dodge=False,
                ax=t5b9_ax) 
    for i in range(len(t5b9_x)):               # bar text 표시
        width = t5b9_y[i]+1.5 
        width_str = str(t5b9_y[i])+'건'
        t5b9_ax.text(width, i, width_str, 
                #  ha='center', va='bottom', 
                 color='green',
                 fontsize=20)                   # bar text 폰크

    t5b9.pyplot(t5b9_fig)  
    ...
    t5t1_map = load_map(base_position) 
