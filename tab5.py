...
def run_tab(): 
    ...
    # t5_cont0.markdown(f"##### 📢 :rainbow[{t5_organ}  민원 분석]") 
    ...
    with tabs[0]: # 월별
        ...
        tabs[0].write(f"📢 민원 건수는 <strong>총 { t5_df0_0_temp[ 'NUMBER' ].sum() } 건</strong> 이며, 최다 발생 기간은 <strong>{ t5_df0_0_temp.iloc[0][ 'DATE' ].strftime('%Y') }년  { t5_df0_0_temp.iloc[0][ 'DATE' ].strftime('%m') }월</strong> <strong>( { t5_df0_0_temp.iloc[0][ 'NUMBER' ] } 건 )</strong> 입니다.       , ", unsafe_allow_html=True) 
    ...
    with tabs[4]: # 데이터
        t5_df4_0, t5_df4_1, t5_df4_2, t5_wc4 = mf.load_df(t5_organ, t5_kind1)  
