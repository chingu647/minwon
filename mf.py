...
def load_wc(organ, choice): 
    ...
    stopwords =['시어','및','조치','예정','민원','처리','해당','통해','방향','후','검토','확인','완료','내','노력','등','위해','위하여','지사','갑례',
                '석기','대하','대하여','대해','대한','도록','토록','하도록','되도록','말씀','수','음','귀하','주신','답변','향','중','향',
                '사항','아래','다음','문의사항','내용','요청','요지','안내','일부','부분','미완료','관내','박준혁','대리','박준혁 대리','관련','저희',
                '것','함','구간','고객']
...
def create_px_bar_month(organ, choice): 
    ...
    fig.update_xaxes(showticklabels = True, 
                     tickformat = '%Y-%m',                # '%d %B (%a)<br>%Y' / '%Y-%b-%d(%a)'
                    #  dtick="M1", 
                     dtick=28 * 24 * 60 * 60 * 1000,   # 한 달 간격을 밀리초로 계산 
                    ) 
    return fig, month_df, point_df, choice_df, wc_data
