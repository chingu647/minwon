...
def run_tab(): 
    ...
    # cont0.markdown(f"##### 📢 :rainbow[{t7_organ}  민원 분석]") 
    ...
    with tabs[0]: # 월별
        ...
        fig0_0, df0_0, df0_1, df0_2, wc0 = mf.create_px_bar_month(t7_organ, t7_kind1) 
    ...
    with tabs[1]: # 유형별 
        fig1_0, df1_0, df1_1, df1_2, wc1 = mf.create_px_pie_kind1(t7_organ, t7_kind1) 
        df1_2_temp = df1_2.sort_values(by='NUMBER', ascending=False) 
        tabs[1].write(f"📚 최다 유형은 <strong>{ df1_2_temp.iloc[0][ f'{t7_kind1}' ] }</strong> 관련으로, " +
                      f"<strong>총 { df1_2_temp.iloc[0][ 'NUMBER' ] } 건 ({ df1_2_temp.iloc[0][ f'NUMBER_pct' ] } %)</strong> 입니다.       , ", unsafe_allow_html=True) 
        # tabs[1].write(f"최다 민원은 <strong>{ df1_2.iloc[0][ f'{t7_kind1}' ] }</strong> 관련으로, <strong>총 { df1_2.iloc[0][ 'NUMBER' ] } 건 ({ df1_2.iloc[0][ f'NUMBER_pct' ] } %)</strong> 입니다.       , ", unsafe_allow_html=True) 
        tabs[1].plotly_chart(fig1_0, use_container_width=True) 


    with tabs[2]: # 팀별
        fig2_0, df2_0, df2_1, df2_2, wc2 = mf.create_px_bar_team(t7_organ, t7_team) 
        df2_2_temp = df2_2.sort_values(by='NUMBER', ascending=False) 
        tabs[2].write(f"📚 최다 처리 팀은 <strong>{ df2_2_temp.iloc[0][ f'{t7_team}' ] }</strong> 으로, " +
    ...
    with tabs[3]: # 노선별
        fig3_0, df3_0, df3_1, df3_2, wc3 = mf.create_px_scatter_road(t7_organ, t7_road) 
        df3_2_temp = df3_2.sort_values(by='NUMBER', ascending=False) 
        tabs[3].write(f"📢 최다 노선은 <strong>{ df3_2_temp.iloc[0][ f'{t7_road}' ] }</strong> 으로, " + 
    ...
    with tabs[4]: # 데이터
        df4_0, df4_1, df4_2, wc4 = mf.load_df(t7_organ, t7_kind1)  
    ...
    cont9.markdown(f"##### 😎 {t7_organ} 민원 :rainbow[노선별로 한눈에 보기] 👀") 

    tabs = st.tabs(['🌍 지 도', '🔎키워드', '💾데이터']) 
    with tabs[0]: 
        pass 
        # 테이블 데이터
        df8_0, df8_1, df8_2, wc8  = mf.load_df(t7_organ, t7_kind1) 

        # map data  
        map_t1 = mf.load_map(t7_base_position, t7_organ, t7_kind1) 
        # mf.load_map_t7_kind1(t7_organ, t7_kind1, t7_base_position) 

    with tabs[1]: 
        fig9_0, df9_0, df9_1, df9_2, wc9 = mf.load_wc(t7_organ, t7_keyword) 
